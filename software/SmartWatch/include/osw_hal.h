...
public:
  ...
  void BMA423shutdown(void);

  //QMC5883L
  bool hasQMC5883L(void);
  void setupQmc5883(void);
  void qmc5883_Sleep(void);
  int qmc5883_GetAzimuth(void);
  int qmc5883_GetX(void);
  int qmc5883_GetY(void);
  int qmc5883_GetZ(void);
  uint8_t qmc5883_ReadReg(uint8_t add);
  void qmc5883_setCalibration(int x_min, int x_max, int y_min, int y_max, int z_min, int z_max);
  void qmc5883_Read(void);

  //Touch
  bool hasCST816S(void);
  void Init_Cst816s(void);
  void Cst816s_Sleep(void);
  uint8_t Cst816s_GetType(void);
  uint16_t Cst816s_GetX();
  uint16_t Cst816s_GetY(void);
  void Cst816s_Read(void);

  // Time
  bool hasPCF8563(void);
...
private:
  ...
  bool _hasBMA423 = false;
  bool _hasQMC5883L = false;
  bool _hasCST816S = false;
